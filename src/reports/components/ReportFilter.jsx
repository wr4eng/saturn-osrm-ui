...
const ReportFilter = ({
  ...
  const loaded = showDates ? from && to && !loading : !loading && deviceIds.length > 0;
  ...
  const onClick = (type) => {
    switch (type) {
      ...
      default:
        if (!showDates) {
          // trip-snap mode: no date range needed, call onShow directly
          onShow({ deviceIds, groupIds });
        } else {
          showReport();
        }
        break;
  ...
  return (
    <div className={classes.filter}>
      ...
      {showDates && selectedOption !== 'schedule' ? (
        <>
          <div className={classes.filterItem}>
